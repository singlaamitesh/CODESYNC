...
async def yjs_websocket(websocket: WebSocket, doc_id: str):
    ...
    # Authorization: the y-websocket room is "codesync-<documentId>", so strip
    # the prefix to recover the real id, then confirm this user may access that
    # document (workspace owner/member, enforced by PocketBase). Without this,
    # any authenticated user could join any document's CRDT room (IDOR).
    real_doc_id = doc_id[len("codesync-"):] if doc_id.startswith("codesync-") else doc_id
    if not await can_access_document(token, real_doc_id):
...
async def editor_websocket(websocket: WebSocket, document_id: str):
    ...
    # Authorization: confirm the user may access this document before accepting
    # (workspace owner/member, enforced by PocketBase). Prevents an
    # authenticated user from subscribing to / triggering AI on others' docs.
    if not await can_access_document(token, document_id):
        await websocket.close(code=4403)  # forbidden
        return
